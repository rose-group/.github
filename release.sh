...
deploy_to_gh_pages() {
    if [ "$PERFORM_GHPAGES_DEPLOY" != true ]; then
        printf "${COLOR_YELLOW}Warning: The feature --gh-pages is not enabled.${COLOR_RESET}\n"
        return
    fi
...
update_changelog() {
    if [ "$PERFORM_CHANGELOG" != true ]; then
        printf "${COLOR_YELLOW}Warning: The feature --changelog is not enabled.${COLOR_RESET}\n"
        return
    fi
...
create_tag_and_push() {
    if [ "$PERFORM_TAG" != true ]; then
        printf "${COLOR_YELLOW}Warning: The feature --tag is not enabled.${COLOR_RESET}\n"
        return
    fi
    printf "\n${COLOR_BLUE}Create tag...${COLOR_RESET}\n"
...
create_release() {
    if [ "$PERFORM_RELEASE" != true ]; then
        printf "${COLOR_YELLOW}Warning: The feature --release is not enabled.${COLOR_RESET}\n"
        return
    fi
